...
หน้าจอสิทธิ์ข้าราชการรองรับเป้าหมายรายเดือนแยกตามหมวดบริการและสิทธิ์รวม 6 ชุด โดยกำหนดได้ทั้งจำนวน visit และยอดเงิน จะเปิดใช้อย่างใดอย่างหนึ่งหรือทั้งสองอย่างก็ได้ ค่าเริ่มต้นคือ `120 visit` ต่อหมวด/สิทธิ์ ข้อมูลเป้าหมายบันทึกใน `data/civil-service-targets.json`

แถบ `ข้าราชการในโรงพยาบาล` นับเฉพาะผู้รับบริการที่ `patient.cid` ตรงกับ `opduser.cid` และแสดงชื่อบัญชี/หน่วยงานของเจ้าหน้าที่ประกอบในตาราง
...
ยอดเบิก สปสช. คิดเฉพาะ case ที่มีรายการ `ADP TELMED` เท่านั้น โดยคิด `50 บาทต่อ case` ไม่ได้นำยอดค่าบริการรวมของ visit มาคิดเป็นยอดเบิก
